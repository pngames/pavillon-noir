...
Missing root element in " << filename 
			...
			const std::string& filename)
		{
			...
			while (shapeNodePtr = const_cast<TiXmlNode*>
				(nodePtr)->IterateChildren("Shape", shapeNodePtr))
			{
...
References element in " << data->name << " in " << 
					filename << ".  Ignoring the Joint." << std::endl;
				delete data;
				return NULL;
			}
			...
			if (NULL != paramNodePtr)
			{
				std::string breakMode = getAttributeString(paramNodePtr, 
					"value");
...
break mode " << breakMode << " in " << filename 
			...
			while (axisNodePtr = const_cast<TiXmlNode*>
				(nodePtr)->IterateChildren("Axis", axisNodePtr))
			{
...
in " << filename << " will be ignored." << std::endl;
						axisNum = 0;
					}
				}
...
value " << data->jointAxisNum << " for GearedMotor \
			...
			if (NULL != paramNodePtr)
			{
				std::string mode = getAttributeString(paramNodePtr, 
					"value");
...
References element in " << data->name << " in " << 
			...
			const std::string& filename)
		{
			...
			if (NULL != paramNodePtr)
			{
				std::string forceType = getAttributeString(paramNodePtr, 
					"value");
...
References element in " << data->name << " in " << 
			...
			const TiXmlNode* nodePtr, const std::string& filename)
		{
			...
			while (transformNodePtr = const_cast<TiXmlNode*>
				(nodePtr)->IterateChildren("Transform", transformNodePtr))
			{
				std::string type = getAttributeString(transformNodePtr, 
					"type");
