...
namespace opal
{
	...
	void ODEJoint::init(const JointData& data)
	{
		...
		// Setup the Solids.
		// Note that this handles the cases when either both are being set 
		// to NULL or both are the same Solid.
		if (data.solid0 == data.solid1 || 
			(data.solid0 && data.solid0->isStatic()) || 
			(data.solid1 && data.solid1->isStatic()))
		{
			Joint::setSolids(NULL, NULL);
			mData.enabled = false;
		}
		else
		{
			Joint::setSolids(data.solid0, data.solid1);
			mData.enabled = true;
		}
	...
	void ODEJoint::setEnabled(bool e)
	{
		...
		else
		{
			...
			{
				// Enable the joint.
				attachODEBodies((ODESolid*)mData.solid0, 
					(ODESolid*)mData.solid1);
			}
			else
			{
	...
	real ODEJoint::calcStress()
	{
		...
		switch(mData.breakMode)
		{
			...
			case ACCUMULATED_MODE:
			{
				...
				Vec3r f1(jf->f1[0], jf->f1[1], jf->f1[2]);
				Vec3r t1(jf->t1[0], jf->t1[1], jf->t1[2]);
				Vec3r f2(jf->f2[0], jf->f2[1], jf->f2[2]);
