...
namespace opal
{
	...
	: Simulator()
	{
		...
		dHashSpaceSetLevels(mRootSpaceID, -5, 9);
	...
	void ODESimulator::internal_setRaycastResult(Solid* solid, 
		const Point3r& intersection, const Vec3r& normal, real distance)
	{
		...
		// If this is the first collided Solid or the new intersection 
		// distance is less than the previous intersection's distance...
		if (NULL == mRaycastResult.solid || (distance < mRaycastResult.distance))
		{
			mRaycastResult.solid = solid;
			mRaycastResult.intersection = intersection;
			mRaycastResult.normal = normal;
			mRaycastResult.distance = distance;
		}
	}
		...
		unsigned int rayContactGroup)
	{
		...
		mRaycastResult.solid = NULL;
		mRaycastResult.intersection.set(0,0,0);
		mRaycastResult.normal.set(1,0,0);
