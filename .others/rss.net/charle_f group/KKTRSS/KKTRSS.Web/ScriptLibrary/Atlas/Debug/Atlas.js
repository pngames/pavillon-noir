...
Web.Data.DataTable = function(tableArray, columns, keys) {
    ...
    this.getItem = function(index) {
        ...
        if (!row) {
            ...
            row = Web.Data.DataRow.isInstanceOfType(rowObject) ? rowObject : new Web.Data.DataRow(_array[index], this, index);
            _rows[index] = row;
        }
        return row;
    }
...
Web.UI.AutoCompleteBehavior = function() {
    ...
    this._onListMouseDown = function() {
        this._setText(window.event.srcElement.innerHTML);
    }
    ...
    this._onKeyDown = function() {
        ...
        if (e.keyCode == 27) {
            ...
            if (_selectIndex != -1) {
                this._setText(_completionListElement.childNodes[_selectIndex].innerHTML);
                e.returnValue = false;
            }
        }
...
Web.UI.Data.ItemView = function(associatedElement) {
    ...
    this.render = function() {
        ...
        if (template) {
            template.createInstance(this.element, this.get_dataContext());
        }
...
Web.WebForms._PageRequest = new function() {
}

Web.WebForms._PageRequest._form = null;
Web.WebForms._PageRequest._scriptManagerID = null;
Web.WebForms._PageRequest._additionalInput = null;
Web.WebForms._PageRequest._onsubmit = null;
Web.WebForms._PageRequest._originalDoPostBack = null;
Web.WebForms._PageRequest._request = null;
Web.WebForms._PageRequest._referencedScripts = [];
Web.WebForms._PageRequest._scriptsToLoad = null;
Web.WebForms._PageRequest._currentLoadingScript = null;
Web.WebForms._PageRequest._XMLScript = null;

Web.WebForms._PageRequest.get_inPostBack = function() {
    return Web.WebForms._PageRequest._request != null;
}

Web.WebForms._PageRequest.abortPostBack = function() {
    if (Web.WebForms._PageRequest.get_inPostBack()) {
        Web.WebForms._PageRequest._request.abort();
        Web.WebForms._PageRequest._request.dispose();
        Web.WebForms._PageRequest._request = null;
    }
}

Web.WebForms._PageRequest._doPostBack = function(eventTarget, eventArgument) {
    Web.WebForms._PageRequest._additionalInput = null;
    ...
    var form = Web.WebForms._PageRequest._form;
    form.__EVENTTARGET.value = eventTarget;
    form.__EVENTARGUMENT.value = eventArgument;
    Web.WebForms._PageRequest._onFormSubmit();
...
Web.WebForms._PageRequest._loadScript = function() {
    var currentScript = Web.WebForms._PageRequest._currentLoadingScript;
    if (currentScript) {
    ...
    var scripts = Web.WebForms._PageRequest._scriptsToLoad;
    if (scripts && (scripts.length > 1)) {
        var scriptSrc = scripts.dequeue();
        var scriptElement = document.createElement('SCRIPT');
        
        Web.WebForms._PageRequest._currentLoadingScript = scriptElement;
        ...
        if (Web.Application.get_type() != Web.ApplicationType.InternetExplorer) {
            ...
            scriptElement.onreadystatechange = Web.WebForms._PageRequest._loadScript;
        }
        scriptElement.type = 'text/javascript';
...
Web.WebForms._PageRequest._onFormElementClick = function() {
    ...
    if (element.tagName == 'INPUT') {
        var type = element.type;
        if (type == 'submit') {
            Web.WebForms._PageRequest._additionalInput = element.name + '=' + encodeURIComponent(element.value);
        }
        else if (type == 'image') {
            var x = window.event.offsetX;
            var y = window.event.offsetY;
            Web.WebForms._PageRequest._additionalInput = element.name + '.x=' + x + '&' + element.name + '.y=' + y;
        }
    }
...
Web.WebForms._PageRequest._onFormSubmit = function() {
    if (Web.WebForms._PageRequest.get_inPostBack()) {
                        if (window.event) {
            window.event.returnValue = false;
        }
    ...
    if (Web.WebForms._PageRequest._onsubmit) {
        continueSubmit = Web.WebForms._PageRequest._onsubmit();
    }
    ...
    formBody.append(Web.WebForms._PageRequest._scriptManagerID + '=Update&');
    ...
    if (Web.WebForms._PageRequest._additionalInput) {
        formBody.append(Web.WebForms._PageRequest._additionalInput);
        Web.WebForms._PageRequest._additionalInput = null;
    }
    ...
    request.completed.add(Web.WebForms._PageRequest._onFormSubmitCompleted);
    request.timeout.add(Web.WebForms._PageRequest._onFormSubmitTimeout);
    request.set_body(formBody.toString());
    
    Web.WebForms._PageRequest._request = request;
    request.invoke();
...
Web.WebForms._PageRequest._onFormSubmitCompleted = function(sender, eventArgs) {
    ...
    if (deltaPanels && deltaPanels.length) {
        deltaPanels = deltaPanels.nodeValue.split(',');
            ...
            var deltaPanelRendering = delta.selectSingleNode('/delta/rendering//span[@id="' + deltaPanelID + '"]');
            deltaPanelRendering = deltaPanelRendering.firstChild.nodeValue;
            
            Web.WebForms._PageRequest._updatePanel(deltaPanelID, deltaPanelRendering);
        }
    }

        var hiddenFields = delta.selectNodes('/delta/rendering//input[@type="hidden"]');
    for (var i = 0; i < hiddenFields.length; i++) {
        var id = hiddenFields[i].attributes.getNamedItem('id').nodeValue;
        var value = hiddenFields[i].attributes.getNamedItem('value').nodeValue;
    ...
    if (styleSheetMarkup) {
        Web.WebForms._PageRequest._updateStyleSheet(styleSheetMarkup.text);
    }
    
            Web.WebForms._PageRequest._XMLScript = delta.selectSingleNode('/delta/xmlScript');
    
        var scripts = delta.selectNodes('/delta/rendering//script[@type="text/javascript"]');
    if (scripts && scripts.length) {
        Web.WebForms._PageRequest._updateScripts(scripts);
    }
    else {
        Web.WebForms._PageRequest._onFormSubmitCompletedCallback();
    }
...
Web.WebForms._PageRequest._onFormSubmitCompletedCallback = function() {
    if (Web.WebForms._PageRequest._XMLScript) {
        Web.Application._update(Web.WebForms._PageRequest._XMLScript.childNodes[0]);
        Web.WebForms._PageRequest._XMLScript = null;
    }
...
Web.WebForms._PageRequest._onFormSubmitTimeout = function(sender, eventArgs) {
    Web.WebForms._PageRequest._request.dispose();
    Web.WebForms._PageRequest._request = null;
}

Web.WebForms._PageRequest._setupAsyncPostBacks = function(form, scriptManagerID) {
    Web.WebForms._PageRequest._form = form;
    Web.WebForms._PageRequest._scriptManagerID = scriptManagerID;

        form._initialAction = form.action;
    
    Web.WebForms._PageRequest._onsubmit = form.onsubmit;
    form.onsubmit = null;
    form.attachEvent('onsubmit', Web.WebForms._PageRequest._onFormSubmit);
    
    form.attachEvent('onclick', Web.WebForms._PageRequest._onFormElementClick);
    
    Web.WebForms._PageRequest._originalDoPostBack = window.__doPostBack;
    if (Web.WebForms._PageRequest._originalDoPostBack) {
        window.__doPostBack = Web.WebForms._PageRequest._doPostBack;
    }
}

Web.WebForms._PageRequest._updatePanel = function(panelID, rendering) {
    var contentPreElement = document.getElementById(panelID + '_Start');
    var contentPostElement = document.getElementById(panelID + '_End');

        var elementsToDestroy = [];
    var nextSibling = contentPreElement.nextSibling;
    while (nextSibling != contentPostElement) {
        elementsToDestroy.add(nextSibling);
        nextSibling = nextSibling.nextSibling;
    }

        var parentNode = contentPreElement.parentNode;
    for (var j = 0; j < elementsToDestroy.length; j++) {
        ...
        if (Web.Application.get_type() == Web.ApplicationType.InternetExplorer) {
        contentPreElement.insertAdjacentHTML('afterEnd', rendering);
    }
    else {
        	    var r = contentPreElement.ownerDocument.createRange();
	    r.setStartBefore(contentPreElement);
	    var parsedNode = r.createContextualFragment(rendering);

	    if (contentPreElement.nextSibling) {
		    parentNode.insertBefore(parsedNode, contentPreElement.nextSibling);
		}
	    else {
...
Web.WebForms._PageRequest._updateScripts = function(scripts) {
    var existingScripts = document.getElementsByTagName('SCRIPT');

    for (var i = existingScripts.length - 1; i >= 0; i--) {
        var scriptNode = existingScripts[i];
        var srcAttribute = scriptNode.attributes.getNamedItem('src');
        ...
        if (src.length) {
            ...
            if (!Web.WebForms._PageRequest._referencedScripts.contains(src)) {
                Web.WebForms._PageRequest._referencedScripts.add(src);
            }
        }
    ...
    if (scriptsToLoad.length != 0) {
        Web.WebForms._PageRequest._scriptsToLoad = scriptsToLoad;
        Web.WebForms._PageRequest._currentLoadingScript = null;
        Web.WebForms._PageRequest._loadScript();
    }
    else {
        Web.WebForms._PageRequest._onFormSubmitCompletedCallback();
    }
...
Web.WebForms._PageRequest._updateStyleSheet = function(cssText) {
