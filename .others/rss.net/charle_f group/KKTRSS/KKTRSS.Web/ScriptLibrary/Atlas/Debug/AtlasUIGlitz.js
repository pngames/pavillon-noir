...
Web.UI.ColorAnimation = function() {
    Web.UI.ColorAnimation.initializeBase(this);
    
    var _interpolateRed;
    var _interpolateGreen;
    var _interpolateBlue;
    
    this.getAnimatedValue = function(percentage) {
        var startColor = this.get_startValue();
        var endColor = this.get_endValue();
        
        var red = startColor.get_red();
        var green = startColor.get_green();
        var blue = startColor.get_blue();
        
        if (_interpolateRed) {
            red = Math.round(Web.UI.Glitz.interpolate(red, endColor.get_red(), percentage));
        }
        if (_interpolateGreen) {
            green = Math.round(Web.UI.Glitz.interpolate(green, endColor.get_green(), percentage));
        }
        if (_interpolateBlue) {
            blue = Math.round(Web.UI.Glitz.interpolate(blue, endColor.get_blue(), percentage));
        }
        
        var color = new Web.UI.Color(red, green, blue);
        return color.toString();
    }

    this.getDescriptor = function() {
        var td = Web.UI.ColorAnimation.callBaseMethod(this, 'getDescriptor');
        
        td.addProperty('startValue', Web.UI.Color);
        td.addProperty('endValue', Web.UI.Color);
        return td;
    }
    ...
    this.onStart = function() {
        ...
        _interpolateRed = (startColor.get_red() != endColor.get_red());
        _interpolateGreen = (startColor.get_green() != endColor.get_green());
        _interpolateBlue = (startColor.get_blue() != endColor.get_blue());
    }
}
Type.registerSealedClass('Web.UI.ColorAnimation', Web.UI.InterpolatedAnimation);
Web.TypeDescriptor.addType('script', 'colorAnimation', Web.UI.ColorAnimation);
