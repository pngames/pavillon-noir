...
namespace RSSScreenSaver.UI
{
    ...
    /// /*<T> : IDisposable where T : IItem*/
    public class ItemDescriptionView : IDisposable
    {
       ...
       // private T displayItem;
        private RssItem displayItem = null;

        public RssItem DisplayItem
        {
        ...
        public void Paint(PaintEventArgs e)
        {
            ...
            using (StringFormat textFormat = new StringFormat(StringFormatFlags.LineLimit))
            {
                ...
                using (Brush textBrush = new SolidBrush(Color.FromArgb(textAlpha, ForeColor)))
                {
                    ...
                    {
                        string tmp = "Pas d'appercu\n\n";
                        tmp += displayItem.Link.ToString();
                        e.Graphics.DrawString(tmp, titleFont, textBrush, textRect, textFormat);
                    }
                    else
                        e.Graphics.DrawString(System.Web.HttpUtility.HtmlDecode(displayItem.Description), titleFont, textBrush, textRect, textFormat);
                }
            }
        }
